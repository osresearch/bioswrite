...
)
{
	...
	close(fd);
	return (void*)(page_offset + (uintptr_t) base);
}
...
static inline UINT32 get_region_limit(UINT32 freg)
{
    return (((freg & 0x1fff0000) >> 4) | 0x00000fff);
}


static inline UINT32 get_region_base(UINT32 freg)
{
...
)
{
    ...
    {
        UINT32 freg = get_freg(sp, region);
        UINT32 cur_limit = get_region_limit(freg);
        UINT32 cur_base = get_region_base(freg);
